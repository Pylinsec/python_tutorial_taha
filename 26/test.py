...
class App(Tk):
    ...
    def create_widget(self):
        self.progress = ttk.Progressbar(self, length=500, variable=self.db_var)  # vertical be manay amoodi hast
        # horizontal  ofoqi
        ...
        self.btn_start = Button(self, text="start", command=self.prog_start, bg='yellow', height=5, width=10)
        self.btn_start.pack()
        self.btn_stop = Button(self, text="stop", command=lambda:self.progress.stop(), bg='yellow', height=5, width=10)
        self.btn_stop.pack()
        self.btn_next = Button(self, text="next", command=lambda:self.progress.step(20), bg='yellow', height=5, width=10)
        self.btn_next.pack()
